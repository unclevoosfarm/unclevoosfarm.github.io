...
function ArrowRight() {
  return (
    <svg className="h-4 w-5" fill="none" viewBox="0 0 20 16">
      <path d={svgPaths.p29a30900} fill="#3B82F6" />
    </svg>
  );
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {badge && (
          <div
            className={`absolute top-4 right-4 flex items-center gap-1 px-3 py-1 rounded-full shadow-sm z-20 ${
              badge.type === 'season'
          ...
          >
            {badge.type === 'season' ? (
              <>
                <span className="bg-[#22C55E] rounded-full w-2 h-2" />
                <span className="font-['Quicksand'] font-semibold text-xs">{badge.text}</span>
              </>
            ) : (
              <>
                <SmallStar />
      ...
      <div className="p-8 flex flex-col flex-1 bg-white">
        {/* Title */}
        <h3 className="font-['Fredoka'] text-[#111827] text-2xl mb-2">{name}</h3>

        {/* Price row */}
        <div className="flex items-baseline gap-1 mb-4">
          <span className="font-['Quicksand'] text-[#F97316] text-3xl font-semibold">{price}</span>
          <span className="font-['Quicksand'] text-gray-500 text-sm">{unit}</span>
        </div>

        {/* Description */}
        <p className="font-['Quicksand'] text-gray-600 text-sm leading-relaxed mb-8 flex-1">
          {description}
        </p>
        ...
        <div className="mt-auto w-full pt-6">
          ...
          >
            ...
            <span className="font-['Quicksand'] font-semibold">Order via WhatsApp</span>
          </a>
        </div>
      </div>
    </motion.article>
...
export function HarvestProducts() {
  ...
  return (
    <section id="shop" className="py-20 lg:py-24 bg-[#FFF7ED] relative min-h-[800px]">
      ...
      <div className="max-w-[1280px] mx-auto px-6 lg:px-8 relative z-10">
          ...
          <div className="flex flex-wrap justify-center gap-3 bg-white p-2 rounded-full shadow-sm border border-gray-100">
            {categories.map((cat) => (
              <button
                ...
                className={`px-6 py-2 rounded-full text-sm font-['Quicksand'] font-semibold transition-all duration-300 ${
                  activeCategory === cat
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-20">
            <p className="font-['Quicksand'] text-gray-500">No products found in this category.</p>
          </div>
        )}
      </div>
